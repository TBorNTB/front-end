...
export async function GET(request: NextRequest) {
  try {
    ...
    const elasticUrl = `${getElasticApiUrl(ELASTIC_ENDPOINTS.ELASTIC.PROJECT_SUGGESTION)}?query=${encodeURIComponent(query.trim())}`;
    ...
    console.error('Error in projects suggestions API route:', error);
    return NextResponse.json([], { status: 200 });
  }
}
